...
static void ping_send(struct raw_pcb *raw, const ip_addr_t *addr)
{
  ...
  if ((p->len == p->tot_len) && (p->next == NULL)) {
	...
	if(bReceivedLastOneSend == 0) {
		//ADDLOG_INFO(LOG_FEATURE_MAIN,"Ping lost: (total lost %i, recv %i)\r\n",ping_lost,ping_received);
		ping_lost++;
...
static u8_t ping_recv(void *arg, struct raw_pcb *pcb, struct pbuf *p, const ip_addr_t *addr)
{
  ...
  if ((p->tot_len >= (PBUF_IP_HLEN + sizeof(struct icmp_echo_hdr))) &&
	...
	//ADDLOG_INFO(LOG_FEATURE_MAIN,"Ping recv: %ims (total lost %i, recv %i)\r\n", ms,ping_lost,ping_received);

	//  ADDLOG_INFO(LOG_FEATURE_MAIN,"Ping recv: %ims\r\n", ms);
