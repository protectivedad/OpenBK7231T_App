...
bool DHT_read(dht_t *dht, bool force) {
	...
	ADDLOG_INFO(LOG_FEATURE_SENSOR, "DHT start, pin is %i",(int)dht->_pin);
  ...
  // Go into high impedence state to let pull-up raise data line level and
...
#endif
	{
		...
		if (DHT_expectPulse(dht,false) == TIMEOUT) {
			...
			GLOBAL_INT_RESTORE();
#endif
			ADDLOG_INFO(LOG_FEATURE_SENSOR, "DHT timeout waiting for start signal low pulse.");
		...
		if (DHT_expectPulse(dht, true) == TIMEOUT) {
			...
			GLOBAL_INT_RESTORE();
#endif
			ADDLOG_INFO(LOG_FEATURE_SENSOR, "DHT timeout waiting for start signal high pulse.");
			dht->_lastresult = false;
			return dht->_lastresult;
		}
	...
	for (int i = 0; i < 40; ++i) {
		...
		if ((lowCycles == TIMEOUT) || (highCycles == TIMEOUT)) {
			ADDLOG_INFO(LOG_FEATURE_SENSOR, "DHT timeout waiting for pulse.");
			dht->_lastresult = false;
			return dht->_lastresult;
		}
		data[i / 8] <<= 1;
	...
	if (data[4] == ((data[0] + data[1] + data[2] + data[3]) & 0xFF)) {
		...
		ADDLOG_INFO(LOG_FEATURE_SENSOR, "DHT checksum failure!");
		dht->_lastresult = false;
		return dht->_lastresult;
	}
}
