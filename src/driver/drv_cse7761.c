...
void Cse7761Write(uint32_t reg, uint32_t data) {
	...
	ADDLOG_INFO(LOG_FEATURE_ENERGYMETER, "CSE7761 send %i\n", len);
	for (int i = 0; i < len; i++) {
...
bool Cse7761ReadOnce(uint32_t log_level, uint32_t reg, uint32_t size, uint32_t* value) {
	...
	ADDLOG_INFO(LOG_FEATURE_ENERGYMETER, "CSE7761 rec %i\n", rcvd);
	if (!rcvd) {
...
void CSE7761_RunEverySecond(void) {
   ...
   ADDLOG_INFO(LOG_FEATURE_ENERGYMETER,"CSE7761 syscon %i\n", syscon);
