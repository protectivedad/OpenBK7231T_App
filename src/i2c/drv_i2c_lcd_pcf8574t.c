...
commandResult_t DRV_I2C_LCD_PCF8574_GoTo_Internal(int bClear, const void *context, const char *cmd, const char *args, int cmdFlags) {
	...
	if(dev == 0) {
		ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_GoTo: there is no device on this bus with such addr");
		return CMD_RES_BAD_ARGUMENT;
	}
	ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_GoTo: module %s, address %i", i2cModuleStr, address);
...
commandResult_t DRV_I2C_LCD_PCF8574_Print_Internal(lcdPrintType_t type,const void *context, const char *cmd, const char *args, int cmdFlags) {
	...
	if(dev == 0) {
		ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_Print: there is no device on this bus with such addr");
		return CMD_RES_BAD_ARGUMENT;
	}
	lcd = (i2cDevice_PCF8574_t *)dev;

	ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_Print: module %s, address %i", i2cModuleStr, address);
...
commandResult_t DRV_I2C_LCD_PCF8574_Clear(const void *context, const char *cmd, const char *args, int cmdFlags) {
	...
	if(dev == 0) {
		ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_Clear: there is no device on this bus with such addr");
		return CMD_RES_BAD_ARGUMENT;
	}
	lcd = (i2cDevice_PCF8574_t *)dev;

	ADDLOG_INFO(LOG_FEATURE_I2C,"DRV_I2C_LCD_PCF8574_Clear: module %s, address %i", i2cModuleStr, address);
...
void DRV_I2C_LCD_PCF8574_RunDevice(i2cDevice_t *dev)
{
	...
	if(c == 0){
		...
		PCF8574_LCD_Clear(lcd);
		delay_ms(115);
		//ADDLOG_INFO(LOG_FEATURE_I2C,"Testing lcd" );
